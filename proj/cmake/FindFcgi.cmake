# This file sets:
# - FCGI_INCLUDE_DIRS
# - FCGI_LIBRARIES
# - FCGI_FOUND


FIND_PATH(FCGI_INCLUDE_DIR
    fcgio.h
  PATHS
    /usr/include
    /usr/local/include
    /usr/include/fastcgi
)

FIND_LIBRARY(FCGI_LIB fcgi
  /usr/lib
  /usr/local/lib
  ${USERLIB_ROOT}/lib
)

FIND_LIBRARY(FCGIPP_LIB fcgi++
  /usr/lib
  /usr/local/lib
  ${USERLIB_ROOT}/lib
)

SET(FCGI_FOUND FALSE)

IF(FCGI_INCLUDE_DIR
    AND FCGI_LIB
    AND FCGIPP_LIB)
  SET(FCGI_FOUND TRUE)
  SET(FCGI_LIBRARIES ${FCGI_LIB} ${FCGIPP_LIB})
  SET(FCGI_INCLUDE_DIRS ${FCGI_INCLUDE_DIR})
ENDIF(FCGI_INCLUDE_DIR
    AND FCGI_LIB
    AND FCGIPP_LIB)

IF(FCGI_FOUND)
    IF(NOT Fcgi_FIND_QUIETLY)
      MESSAGE(STATUS "Found the Fcgi libraries at ${FCGI_LIBRARIES}")
      MESSAGE(STATUS "Found the Fcgi headers at ${FCGI_INCLUDE_DIRS}")
    ENDIF (NOT Fcgi_FIND_QUIETLY)
ELSE(FCGI_FOUND)
    IF(Fcgi_FIND_REQUIRED)
      MESSAGE(FATAL_ERROR "Could NOT find required fcgi libraries")
    ENDIF(Fcgi_FIND_REQUIRED)
ENDIF(FCGI_FOUND)
