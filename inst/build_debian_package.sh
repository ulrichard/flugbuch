...
mkdir builddeb
cd builddeb
for DISTRIBUTION in jaunty karmic lucid maverick natty oneric
do
	...
	cd flugbuch2-$FLUGBUCHVERSIONSTR
		sed -i  -e "s/jaunty/${DISTRIBUTION}/" -e "s/karmic/${DISTRIBUTION}/" -e "s/maverick/${DISTRIBUTION}/" -e "s/natty/${DISTRIBUTION}/" -e "s/oneric/${DISTRIBUTION}/" debian/changelog
	sed -i  -e "s/jaunty/${DISTRIBUTION}/" -e "s/karmic/${DISTRIBUTION}/" -e "s/maverick/${DISTRIBUTION}/" -e "s/natty/${DISTRIBUTION}/" -e "s/oneric/${DISTRIBUTION}/" debian/changelog
